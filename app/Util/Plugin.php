...
class Plugin
{
    ...
    public static function getPluginLog(string $pluginName): string
    {
        ...
        // 不再展示 hook 调试日志的回退内容，空日志则返回空字符串
        return "";
    }
    ...
    public static function ClearPluginLog(string $pluginName): bool
    {
        ...
        // 同时清理全局 hook 调试日志中与该插件相关的条目
        $global = BASE_PATH . '/runtime/hook_debug.log';
        if (file_exists($global)) {
            $lines = @file($global, FILE_IGNORE_NEW_LINES) ?: [];
            $kept = [];
            foreach ($lines as $line) {
                $has = false;
                if (str_contains($line, "plugin={$pluginName}") || str_contains($line, "plugin='{$pluginName}'")) {
                    $has = true;
                }
                if (!$has && str_contains($line, "Plugin '{$pluginName}'")) {
                    $has = true;
                }
                if (!$has && str_contains($line, "App\\Plugin\\{$pluginName}\\")) {
                    $has = true;
                }
                if (!$has) {
                    $kept[] = $line;
            ...
            @file_put_contents($global, implode(PHP_EOL, $kept) . PHP_EOL);
        }
        return $ok;
    }
