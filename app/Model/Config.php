...
class Config extends Model
{
    ...
    public static function get(string $key): string
    {
        ...
        if (Redis::inst()->isAvailable()) {
            $val = Redis::inst()->get($cacheKey);
            if ($val !== false) {
                Context::set($cacheKey, $val);
                return (string)$val;
            }
        } else {
        ...
        if (Redis::inst()->isAvailable()) {
            Redis::inst()->set($cacheKey, $cfg->value);
        }
    ...
    public static function put(string $key, string|int $value): void
    {
        ...
        if (Redis::inst()->isAvailable()) {
            Redis::inst()->set("_DB_CONFIG_" . $key, $value);
        }
