...
class Shop implements \App\Service\Shop
{
    ...
    public function getItem(int|string $commodityId, ?User $user = null, ?UserGroup $group = null): array
    {
        ...
        if ($business) {
            $commodity->service_url = $business->service_url;
            $commodity->service_qq = $business->service_qq;
        } elseif ($commodity->owner) {
            $ownerBusiness = Business::query()->where("user_id", $commodity->owner->id)->first();
            if ($ownerBusiness) {
                $commodity->service_url = $ownerBusiness->service_url;
                $commodity->service_qq = $ownerBusiness->service_qq;
            }
        }
