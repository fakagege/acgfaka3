...
class Master extends User
{
    ...
    public function setCategory(): array
    {
        ...
        if ($map['id'] != 0) {
            $userCategory = UserCategory::query()->where("user_id", $map['user_id'])->find($map['id']);
            if (!$userCategory) {
                throw new JSONException("设置错误，请刷新网页");
            }
            $map['category_id'] = $userCategory->category_id;
        }

        $save = new Save(UserCategory::class);
        $save->setMap($map, ['name', 'status']);
        $save->addForceMap('user_id', $map['user_id']);
        $save->addForceMap('category_id', (int)$map['category_id']);
    ...
    public function setCommodity(): array
    {
        ...
        $map['user_id'] = $this->getUser()->id;
        if ($map['id'] != 0) {
            $userCommodity = UserCommodity::query()->where("user_id", $map['user_id'])->find($map['id']);
            if (!$userCommodity) {
                throw new JSONException("设置错误，请刷新网页");
            }
            $map['commodity_id'] = $userCommodity->commodity_id;
        }
        ...
        $save->addForceMap('user_id', $map['user_id']);
        $save->addForceMap('commodity_id', (int)$map['commodity_id']);
