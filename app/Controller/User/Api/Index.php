...
class Index extends User
{
    ...
    public function card(): array
    {
        ...
        // 自定义排序：根据 draft 尾号字符排序 (0-9 -> A-Z -> a-z)
        if (!empty($data['list'])) {
            usort($data['list'], function ($a, $b) {
                $draftA = trim((string)$a['draft']);
                $draftB = trim((string)$b['draft']);
                ...
                $getLastChar = function ($str) {
                    ...
                    // 如果没有数字字母，尝试取最后一个非空字符
                    return mb_substr($str, -1);
                };

                $charA = $getLastChar($draftA);
                $charB = $getLastChar($draftB);
    ...
    function valuation(): array
    {
        ...
        return $this->json(data: ["price" => $price, "card_count" => $stock]);
    }
