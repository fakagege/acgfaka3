...
class Plugin extends Manage
{
    ...
    public function setConfig(Request $request): array
    {
        ...
        if (isset($map['STATUS'])) {
            ...
            // Manual configuration update to bypass _plugin_start/_plugin_stop issues
            $configPath = BASE_PATH . "/app/Plugin/{$id}/Config/Config.php";
            ...
            if ($map['STATUS'] == 1) {
                ...
                $pluginName = $plugin[\App\Consts\Plugin::NAME] ?? $id;
                ManageLog::log($this->getManage(), "启用了插件：{$pluginName}");
                return $this->json(200, "插件已启动");
            } else {
                \Kernel\Util\Plugin::runHookState($id, \Kernel\Annotation\Plugin::STOP);
                $pluginName = $plugin[\App\Consts\Plugin::NAME] ?? $id;
                ManageLog::log($this->getManage(), "停用了插件：{$pluginName}");
