...
class Install extends User
{
    ...
    public function step(): string
    {
        ...
        $data['ext']['redis'] = extension_loaded("redis");
        ...
        if ($data['php_version'] < 8) {
            ...
            $requiredExt = $data['ext'];
            unset($requiredExt['redis']);
...
EOF;
        ...
        // Redis配置写入
        $redis_enable = !empty($map['redis_host']) ? 'true' : 'false';
        $redis_host = $map['redis_host'] ?: '127.0.0.1';
        $redis_port = $map['redis_port'] ?: 6379;
        $redis_password = $map['redis_password'] ? "'{$map['redis_password']}'" : 'null';
        $redis_db = $map['redis_db'] ?: 0;
...
EOF;
        ...
        file_put_contents(BASE_PATH . '/config/redis.php', $redis_conf);

        if (file_put_contents(BASE_PATH . '/config/database.php', $db_conf) === false) {
