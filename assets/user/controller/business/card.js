...
VIP-2025-0821-XYZ
        ...
        <div class="card-body">
          ...
          <p class="text-muted mb-3">
            一行一个，支持两种格式（推荐使用第一种）：<br>
            1. <span class="text-dark fw-bold">预告信息 --- 卡密本体 --- 自选加价金额(可选)</span><br>
            2. <span class="text-dark fw-bold">卡密本体 ║ 预告信息 ║ 自选加价金额(可选)</span>
          </p>
          ...
          <div class="translucent border rounded p-3">
<pre class="mb-0" style="white-space: pre-wrap; word-break: break-all;">
大区:神境之地--等级:100---账号:testname--密码:testpassword123---5.5
地区:美区·时长:12个月---ACC_US_12M_9F2K-7QPA-88XZ---20
ACC_JP_6M_0KLD-22MM-PP31║地区:日区·时长:6个月
</pre>
          </div>

          <div class="alert alert-warning mt-3 mb-0 small">
            ⚠️ 旧格式必须使用特殊符号 <strong>“║”</strong>（U+2551），新格式使用三个减号 <strong>“---”</strong>
          </div>
        </div>
      </div>`);
                            }
    ...
    table = new Table("/user/api/card/data", "#user-card-table");
    table.setUpdate("/user/api/card/edit");
    table.setColumns([
        {checkbox: true},
        {
    ...
    table.setPagination(10, [10, 30, 50, 100])
    table.setSearch([
    ...
    $('.button-del').click(() => {
        ...
        message.ask("注意，删除卡密后无法恢复", () => {
            util.post("/user/api/card/del", {list: data}, res => {
                message.success("删除成功")
                table.refresh();
            });
        });
    ...
    $('.button-lock').click(() => {
        ...
        message.ask("您确定要锁定选中的卡密吗？", () => {
            util.post("/user/api/card/lock", {list: data}, res => {
    ...
    $('.button-sell').click(() => {
        ...
        message.ask("您确定要手动出售选中的卡密吗？", () => {
            util.post("/user/api/card/sell", {list: data}, res => {
    ...
    $('.button-export').click(function () {
        component.popup({
            ...
            submit: (data, index) => {
                ...
                let url = "/user/api/card/export?" + query + "&equal-" + state.field + "=" + state.value;
